...
async function fixStudentInstructorStatus() {
  ...
  try {
    ...
    console.log(
      `📊 Encontradas ${acceptedRelations.length} relaciones con status "accepted"`,
    );
    ...
    if (acceptedRelations.length > 0) {
      ...
      acceptedRelations.forEach((rel) => {
        console.log(
          `  - ${rel.student.name} -> ${rel.instructor.user.name} (${rel.id})`,
        );
      });
    ...
    const activeCount = await prisma.studentInstructor.count({
      where: { status: "active" },
    });
    const pendingCount = await prisma.studentInstructor.count({
      where: { status: "pending" },
    });
    const acceptedCount = await prisma.studentInstructor.count({
