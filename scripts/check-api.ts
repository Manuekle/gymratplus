...
const modelUsage: Record<string, {
  count: number;
  files: string[];
}> = Object.fromEntries(PRISMA_MODELS.map(model => [model, { count: 0, files: [] }]));
...
async function checkFileForPrismaUsage(filePath: string) {
  try {
    ...
    const prismaClientPattern = /(?:import\s+.*\s+from\s+['"]@prisma\/client['"]|const\s+prisma\s*=\s*new\s+PrismaClient)/;
    ...
    PRISMA_MODELS.forEach(model => {
      ...
      const patterns = [
        ...
        new RegExp(`const\s+[a-zA-Z0-9_]*\s*=\s*await\s+[a-zA-Z0-9_]*\.${model}\\b`, 'g'), // With const assignment
      ];
