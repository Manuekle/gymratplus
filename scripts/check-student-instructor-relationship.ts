...
async function checkStudentInstructorRelationship() {
  try {
    ...
    // Agrupar por status
    const relationsByStatus = allRelations.reduce(
      (acc, relation) => {
    ...
    Object.entries(relationsByStatus).forEach(([status, relations]) => {
      ...
      relations.forEach((relation) => {
        console.log(
          `  - Estudiante: ${relation.student.name} (${relation.student.email})`,
        );
        console.log(
          `    Instructor: ${relation.instructor.user.name} (${relation.instructor.user.email})`,
        );
    ...
    activeRelations.forEach((relation) => {
      console.log(
        `  - Estudiante: ${relation.student.name} (${relation.studentId})`,
      );
      console.log(
        `    Instructor: ${relation.instructor.user.name} (${relation.instructor.userId})`,
      );
