...
async function testWorkoutAssignment() {
  try {
    ...
    if (!validRelation) {
      console.log(
        "❌ No se encontró ninguna relación instructor-estudiante válida",
      );
      return;
    }
    ...
    console.log(
      `  - Estudiante: ${validRelation.student.name} (${validRelation.student.id})`,
    );
    console.log(
      `  - Instructor: ${validRelation.instructor.user.name} (${validRelation.instructor.user.id})`,
    );
    ...
    console.log(
      `  - Relación encontrada: ${studentRelationship ? "✅" : "❌"}`,
    );
