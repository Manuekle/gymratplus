...
export function WorkoutAssignmentDialog({
  ...
  const { assignWorkout, isAssigning } = useInstructorWorkouts();
  ...
  const handleSubmit = async () => {
    ...
    if (!studentId) {
      toast.error("Selecciona un estudiante para asignar la rutina");
      return;
    }
    setSubmitting(true);
    try {
      // 1. Crear la rutina personalizada
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.details || "Error creando rutina");
      }
      const workout = await response.json();
      // 2. Asignar la rutina al estudiante
      ...
      console.error("Error creando/asignando rutina:", error);
      toast.error("Error al crear o asignar el entrenamiento");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent>
        <DialogHeader className="pt-2">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            Sigue los pasos para crear y asignar un plan de entrenamiento
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            {/* Fecha de vencimiento y notas opcionales */}
            <div className="space-y-2">
              <ExpiredDatePicker
                value={dueDate}
            ...
            <div className="flex justify-between pt-4">
              <Button
                ...
                className="text-xs"
                onClick={() => setStep(2)}
                disabled={submitting || isAssigning}
              >
                Anterior
              </Button>
              <Button
