...
export const useGoals = () => {
  ...
  const fetchGoals = useCallback(async (type?: GoalType, status?: GoalStatus) => {
    setIsLoading(true)
  ...
  const addProgressUpdate = useCallback(async (goalId: string, progressData: Omit<GoalProgress, "goalId">) => {
    setIsLoading(true)
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Error al añadir actualización de progreso")
      }
  ...
  const fetchProgressUpdates = useCallback(async (goalId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || "Error al obtener actualizaciones de progreso")
      }
