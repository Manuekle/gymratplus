...
export function useWorkouts() {
  ...
  const createWorkout = useCallback(
    async (
      workoutData: Omit<
        Workout,
        "id" | "createdAt" | "updatedAt" | "createdById" | "exercises"
      >,
      exercises: Omit<WorkoutExercise, "id" | "workoutId" | "exercise">[],
    ) => {
      setIsSaving(true);
      try {
        const res = await fetch("/api/workouts", {
        ...
        toast.error(
          error instanceof Error ? error.message : "Error al crear la rutina",
        );
        throw error;
      } finally {
        setIsSaving(false);
    ...
    async (workoutId: string, workoutName: string) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          globalPersonalWorkouts = globalPersonalWorkouts.filter(
