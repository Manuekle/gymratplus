...
export async function getAdminStats() {
    const [usersCount, exercisesCount, activeSessionsCount, financialStats, analyticsData] = await Promise.all([
        prisma.user.count(),
...
export async function createFood(prevState: any, formData: FormData) {
    ...
    const { name, calories, protein, carbs, fat, serving, category } = validatedFields.data;
...
export async function updateFood(id: string, prevState: any, formData: FormData) {
    const validatedFields = FoodSchema.safeParse({
        name: formData.get("name"),
        calories: formData.get("calories"),
        protein: formData.get("protein"),
...
export async function getFinancialStats() {
    ...
    const totalRevenue = allPaidInvoices.reduce((acc, inv) => acc + inv.amount, 0);
    ...
    const monthlyRevenue = monthlyPaidInvoices.reduce((acc, inv) => acc + inv.amount, 0);
...
export async function getAnalyticsData() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const monthYear = d.toLocaleDateString("en-US", { month: "short", year: "2-digit" }); // e.g., "Jan 24"
        monthlyData[monthYear] = { name: monthYear, revenue: 0, users: 0 };
    }

    // Process Revenue
    rawRevenue.forEach((entry) => {
        const monthYear = new Date(entry.billingDate).toLocaleDateString("en-US", { month: "short", year: "2-digit" });
    ...
    rawUsers.forEach((entry) => {
        const monthYear = new Date(entry.createdAt).toLocaleDateString("en-US", { month: "short", year: "2-digit" });
        if (monthlyData[monthYear]) {
...
export async function createAdminWorkout(prevState: any, formData: FormData) {
    ...
    if (!adminEmail) return { message: "Server Error: No Admin Email Configured" };

    const adminUser = await prisma.user.findUnique({ where: { email: adminEmail } });
    if (!adminUser) return { message: "Server Error: Admin User not found in DB" };
