...
export default function InstructorRegistrationPage() {
    ...
    if (success === "true" && subscriptionId) {
      ...
      if (instructorData) {
        ...
        fetch("/api/payment/activate", {
          ...
          .then((res) => res.json())
          .then(async (activateData) => {
            if (activateData.success) {
              // Luego registrar instructor
      ...
      sessionStorage.removeItem("instructorRegistrationData");
      // Limpiar la URL
      window.history.replaceState({}, "", "/dashboard/instructor/register");
    }
  }, [update, router]);
  ...
  if (session?.user?.isInstructor && hasSubscription) {
    return (
      ...
      <Card className="w-full overflow-hidden">
        <CardHeader className="pb-4">
          <CardTitle className="text-2xl font-semibold tracking-heading">
            {step === 1 ? "Registro de Instructor" : "Configuración de Pago"}
          </CardTitle>
          <CardDescription className="text-muted-foreground text-xs">
            {step === 1
              ? "Completa tu información profesional"
              : "Selecciona tu plan y método de pago"}
          </CardDescription>
        </CardHeader>
