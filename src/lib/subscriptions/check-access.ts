...
import { canAccessFeature, FEATURES, FeatureName, SubscriptionTier } from './feature-gates'
...
export async function requireFeature(featureName: FeatureName) {
    ...
    const userTier = (session.user as any).subscriptionTier || SubscriptionTier.FREE

    if (!canAccessFeature(userTier, featureName)) {
        const feature = FEATURES[featureName]
        throw new Error(`Upgrade required - This feature requires ${feature.tier} tier`)
    }
 ...
 * Use in API routes and server components
 */
export async function hasFeatureAccess(featureName: FeatureName): Promise<boolean> {
    try {
        const session = await getServerSession(authOptions)
